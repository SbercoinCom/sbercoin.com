Gitian building
================
*Setup instructions for a Gitian build of Sbercoin.com Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Sbercoin.com
...
- [Building Sbercoin.com Core](#building-sbercoin-core)
...
Clone the git repositories for Sbercoin.com and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/SbercoinCom/sbercoin.com --recursive
git clone https://github.com/SbercoinCom/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/sbercoin.com/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the Sbercoin.com repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Sbercoin Core
----------------

To build Sbercoin.com Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Sbercoin.com repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sbercoin.com/.git/
...
For example:
```bash
URL=https://github.com/laanwj/sbercoin.com.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit sbercoin.com=${COMMIT} --url sbercoin.com=${URL} ../sbercoin.com/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sbercoin.com=${COMMIT} --url sbercoin.com=${URL} ../sbercoin.com/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sbercoin.com=${COMMIT} --url sbercoin.com=${URL} ../sbercoin.com/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-sbercoin:
./bin/gbuild --commit sbercoin.com=${COMMIT},cpp-eth-sbercoin=${ETHCOMMIT} --url sbercoin.com=${URL},cpp-eth-sbercoin=${ETHURL} ../sbercoin.com/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the Sbercoin.com git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../sbercoin.com/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/SbercoinCom/sbercoin-detached-sigs.git

BTCPATH=/some/root/path/sbercoin.com
SIGPATH=/some/root/path/sbercoin-detached-sigs

./bin/gbuild --url sbercoin.com=${BTCPATH},signature=${SIGPATH} ../sbercoin.com/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sbercoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sbercoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/sbercoin-osx-build.assert
```
