...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // sbercoin
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // sbercoin

    if(blockindex->IsProofOfStake()){
        result.pushKV("prevoutStakeHash", blockindex->prevoutStake.hash.GetHex()); // sbercoin
        result.pushKV("prevoutStakeVoutN", (int64_t)blockindex->prevoutStake.n); // sbercoin
    }
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // sbercoin
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // sbercoin
...
UniValue transactionReceiptToJSON(const SbercoinTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
static std::vector<RPCResult> MempoolEntryDescription() { return {
    ...
    RPCResult{RPCResult::Type::NUM, "size", "(DEPRECATED) same as vsize. Only returned if sbercoind is started with -deprecatedrpc=size\n"
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination sbercoinSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(sbercoinSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&sbercoinSenderAddress);
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationinfoforaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Sbercoin.com address string"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
    ...
    SbercoinDelegation sbercoinDelegation;
    Delegation delegation;
    uint160 address = uint160(*pkhash);
    if(!sbercoinDelegation.GetDelegation(address, delegation)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegation");
    }
    bool verified = sbercoinDelegation.VerifyDelegation(address, delegation);
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationsforstaker",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Sbercoin.com address string for staker"},
                },
               RPCResult{
            RPCResult::Type::ARR, "", "",
                {
    ...
    if(!sbercoinDelegation.FilterDelegationEvents(events, filter)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegations for staker");
    }
    std::map<uint160, Delegation> delegations = sbercoinDelegation.DelegationsFromEvents(events);
                        ...
                        {RPCResult::Type::OBJ, "scriptPubKey", "",
                            {
                                ...
                                {RPCResult::Type::ARR, "addresses", "array of Sbercoin.com addresses",
                                    {{RPCResult::Type::STR, "address", "Sbercoin.com address"}}},
                            }},
