...
static const CAmount SINGLE_STEP = 0.0000001*COIN;
}
...
void RemoveDelegationPage::setModel(WalletModel *_model)
{
    ...
    // update the display unit, to not use the default ("SBER")
    updateDisplayUnit();
}
...
void RemoveDelegationPage::on_gasInfoChanged(quint64 blockGasLimit, quint64 minGasPrice, quint64 nGasPrice)
{
    ...
    ui->labelGasPrice->setToolTip(tr("Gas price: SBER price per gas unit. Default = %1, Min = %2").arg(QString::fromStdString(FormatMoney(DEFAULT_GAS_PRICE))).arg(QString::fromStdString(FormatMoney(minGasPrice))));
...
void RemoveDelegationPage::on_removeDelegationClicked()
{
    if(m_model && m_clientModel)
    {
                ...
                (details.w_create_in_mempool || !details.w_create_in_main_chain))
        {
            ...
            QMessageBox::information(this, tr("Remove delegation for address"), tr("Please wait for the create contract delegation transaction be confirmed or abandon the transaction.<br /><br />Transaction ID: %1").arg(txid));
            return;
        }
                ...
                (details.w_remove_in_mempool || !details.w_remove_in_main_chain))
        {
            ...
            QMessageBox::information(this, tr("Remove delegation for address"), tr("Please wait for the remove contract delegation transaction be confirmed or abandon the transaction.<br /><br />Transaction ID: %1").arg(txid));
            return;
        }
        ...
        if(!details.c_entry_exist)
        {
            ...
            QMessageBox::information(this, tr("Remove delegation for address"), tr("Delegation already removed. <br />The delegation for the address will be removed from the wallet list."));
