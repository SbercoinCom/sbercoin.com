...
bool ToHash160(const std::string& strSbercoinAddress, std::string& strHash160)
{
    CTxDestination sbercoinAddress = DecodeDestination(strSbercoinAddress);
    if(!IsValidDestination(sbercoinAddress))
        return false;
    const PKHash * keyid = boost::get<PKHash>(&sbercoinAddress);
    if(keyid){
...
bool ToSbercoinAddress(const std::string& strHash160, std::string& strSbercoinAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    PKHash keyid(key);
    CTxDestination sbercoinAddress = keyid;
    if(IsValidDestination(sbercoinAddress)){
        strSbercoinAddress = EncodeDestination(sbercoinAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToSbercoinAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToSbercoinAddress(tokenEvent.receiver, tokenEvent.receiver);
