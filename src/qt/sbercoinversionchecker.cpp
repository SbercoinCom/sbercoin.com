...
#define paternVersion "sbercoin-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

SbercoinVersionChecker::SbercoinVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

SbercoinVersionChecker::~SbercoinVersionChecker()
{

}

bool SbercoinVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> SbercoinVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(SBER_RELEASES)));
    QEventLoop event;
...
Version SbercoinVersionChecker::getMaxReleaseVersion()
{
