...
void AddDelegationPage::on_addDelegationClicked()
{
    if(m_model)
    {
        ...
        if(details.c_entry_exist)
        {
            ...
            {
                QMessageBox::warning(this, tr("Set delegation for address"), tr("The address is delegated to the staker:<br />") + QString::fromStdString(details.c_staker_address));
                return;
            }
            else
            {
                // Add the delegation to the wallet
                QMessageBox::information(this, tr("Set delegation for address"), tr("Delegation already present. <br />The delegation for the address will be added in the wallet list."));
