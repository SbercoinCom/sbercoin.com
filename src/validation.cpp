...
#if defined(NDEBUG)
# error "Sbercoin.com cannot be compiled without assertions."
#endif
...
std::unique_ptr<SbercoinState> globalState;
...
bool fAddressIndex = false; // sbercoin
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        uint64_t minGasPrice = sbercoinDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = sbercoinDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        SbercoinTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractSbercoinTX resultConverter;
        if(!converter.extractionSbercoinTransactions(resultConverter)){
        ...
        std::vector<SbercoinTransaction> sbercoinTransactions = resultConverter.first;
        std::vector<EthTransactionParams> sbercoinETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(SbercoinTransaction sbercoinTransaction : sbercoinTransactions){
            sumGas += sbercoinTransaction.gas() * sbercoinTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, sbercoinTransaction.gasPrice());
            } else {
                txMinGasPrice = sbercoinTransaction.gasPrice();
            }
            VersionVM v = sbercoinTransaction.getVersion();
            if(v.format!=0)
            ...
            if(sbercoinTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(sbercoinTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(sbercoinTransaction.gas() > UINT32_MAX)
            ...
            gasAllTxs += sbercoinTransaction.gas();
            ...
            if(v.rootVM!=0 && (uint64_t)sbercoinTransaction.gasPrice() < minGasPrice)
        ...
        if(!CheckMinGasPrice(sbercoinETP, minGasPrice))
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-txns-small-gasprice");

        if(count > sbercoinTransactions.size())
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s SBER additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // sbercoin
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // sbercoin
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    SbercoinTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = SbercoinTransaction(nAmount, 1, dev::u256(gasLimit), opcode, dev::u256(0));
    }
    else
    {
        callTransaction = SbercoinTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<SbercoinTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path sbercoinDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(sbercoinDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(sbercoinDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(SbercoinTransaction& tx : txs){
        //validate VM version
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, SbercoinTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool SbercoinTxConverter::extractionSbercoinTransactions(ExtractSbercoinTX& sbercointx){
    ...
    std::vector<SbercoinTransaction> resultTX;
    ...
    sbercointx = std::make_pair(resultTX, resultETP);
    return true;
}

bool SbercoinTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool SbercoinTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
SbercoinTransaction SbercoinTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    SbercoinTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = SbercoinTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = SbercoinTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t SbercoinTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // sbercoin
    SbercoinDGP sbercoinDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setSbercoinSchedule(sbercoinDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = sbercoinDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = sbercoinDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = sbercoinDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // sbercoin
...
///////////////////////////////////////////////////////////////////////////////////////// sbercoin
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            SbercoinTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractSbercoinTX resultConvertSbercoinTX;
            if(!convert.extractionSbercoinTransactions(resultConvertSbercoinTX)){
            ...
            if(!CheckMinGasPrice(resultConvertSbercoinTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertSbercoinTX.first, blockGasLimit, pindex->pprev);
            ...
            for(SbercoinTransaction& qtx : resultConvertSbercoinTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertSbercoinTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertSbercoinTX.first[k].from(),
                        resultConvertSbercoinTX.first[k].to(),
                        ...
                        resultConvertSbercoinTX.first[k].getNVout()
                    });
                }
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().MineBlocksOnDemand()) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(919748415000 + ((pindex->nHeight - 5000) * 300)) * COIN){
...
bool CChainState::ConnectTip(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // sbercoin
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // sbercoin
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // sbercoin
            globalState->setRootUTXO(oldHashUTXORoot); // sbercoin
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // sbercoin
        uint32_t sizeBlockDGP = sbercoinDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Sbercoin.com PoS we need to sync this after every block to ensure txdb is populated for
...
CAmount GetTxGasFee(const CMutableTransaction& _tx)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        SbercoinTxConverter convert(tx, NULL, NULL, contractflags);
