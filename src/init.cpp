 ...
 */
static const char* BITCOIN_PID_FILENAME = "sbercoind.pid";
...
void Shutdown(NodeContext& node)
{
    ...
    util::ThreadRename("sbercoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/SbercoinCom/sbercoin.com>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateSbercoin");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path sbercoinStateDir = GetDataDir() / "stateSbercoin";
                bool fStatus = fs::exists(sbercoinStateDir);
                const std::string dirSbercoin(sbercoinStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsSbercoinstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<SbercoinState>(new SbercoinState(dev::u256(0), SbercoinState::openDB(dirSbercoin, hashDB, dev::WithExisting::Trust), dirSbercoin, existsSbercoinstate));
                dev::eth::ChainParams cp(chainparams.EVMGenesisInfo());
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(sbercoinStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setSbercoinSchedule(sbercoinDGP.getGasSchedule(::ChainActive().Height() + (::ChainActive().Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
