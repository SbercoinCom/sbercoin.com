...
#include <sbercoin/sbercoinstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

SbercoinState::SbercoinState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = SbercoinState::openDB(_path + "/sbercoinDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

SbercoinState::SbercoinState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute SbercoinState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, SbercoinTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createSbercoinAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            sbercoin::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, SbercoinTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, SbercoinTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}

std::unordered_map<dev::Address, Vin> SbercoinState::vins() const // temp
{
...
void SbercoinState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* SbercoinState::vin(dev::Address const& _a) const
{
    return const_cast<SbercoinState*>(this)->vin(_a);
}

Vin* SbercoinState::vin(dev::Address const& _addr)
{
...
// void SbercoinState::commit(CommitBehaviour _commitBehaviour)
// {
...
void SbercoinState::kill(dev::Address _addr)
{
...
void SbercoinState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void SbercoinState::deleteAccounts(std::set<dev::Address>& addrs){
...
void SbercoinState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void SbercoinState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void SbercoinState::validateTransfersWithChangeLog(){
...
void SbercoinState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!SbercoinState::addressInUse(delegationsAddress)){
        SbercoinState::createContract(delegationsAddress);
        SbercoinState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)});
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
