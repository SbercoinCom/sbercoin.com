...
class SbercoinDelegationPriv
{
public:
    SbercoinDelegationPriv():
        m_pfDelegations(0),
    ...
    virtual ~SbercoinDelegationPriv()
    {
        if(m_pfDelegations)
...
SbercoinDelegation::SbercoinDelegation():
    priv(0)
{
    priv = new SbercoinDelegationPriv();
}
...
bool SbercoinDelegation::GetDelegation(const uint160 &address, Delegation &delegation) const
{
...
bool SbercoinDelegation::VerifyDelegation(const uint160 &address, const Delegation &delegation)
{
...
bool SbercoinDelegation::FilterDelegationEvents(std::vector<DelegationEvent> &events, const IDelegationFilter &filter, int fromBlock, int toBlock, int minconf) const
{
...
std::map<uint160, Delegation> SbercoinDelegation::DelegationsFromEvents(const std::vector<DelegationEvent> &events)
{
...
void SbercoinDelegation::UpdateDelegationsFromEvents(const std::vector<DelegationEvent> &events, std::map<uint160, Delegation> &delegations)
{
...
bool SbercoinDelegation::ExistDelegationContract() const
{
    // Delegation contract exist check
    return globalState && globalState->addressInUse(priv->delegationsAddress);
}

std::string SbercoinDelegation::BytecodeRemove()
{
    return DelegationABI()["removeDelegation"].selector();
}

bool SbercoinDelegation::BytecodeAdd(const std::string &hexStaker, const int &fee, const std::vector<unsigned char> &PoD, std::string &datahex, std::string &errorMessage)
{
