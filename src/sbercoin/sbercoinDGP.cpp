#include <sbercoin/sbercoinDGP.h>
...
void SbercoinDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool SbercoinDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule SbercoinDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t SbercoinDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t SbercoinDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t SbercoinDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t SbercoinDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool SbercoinDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void SbercoinDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void SbercoinDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void SbercoinDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void SbercoinDGP::createParamsInstance(){
...
dev::Address SbercoinDGP::getAddressForBlock(unsigned int blockHeight){
...
void SbercoinDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void SbercoinDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void SbercoinDGP::parseStorageOneUint64(uint64_t& value){
...
void SbercoinDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule SbercoinDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
