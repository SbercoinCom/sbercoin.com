...
SbercoinDelegation& GetSbercoinDelegation()
{
    static SbercoinDelegation sbercoinDelegation;
    return sbercoinDelegation;
}
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, BlockValidationState& state, const CTransaction& tx, unsigned int nBits, uint32_t nTimeBlock, const std::vector<unsigned char>& vchPoD,  const COutPoint& headerPrevout, uint256& hashProofOfStake, uint256& targetProofOfStake, CCoinsViewCache& view)
{
    ...
    if (nHeight >= nOfflineStakeHeight && !Params().GetConsensus().delegationsAddress.IsNull())
    {
        ...
        SbercoinDelegation& sbercoinDelegation = GetSbercoinDelegation();
        if(!sbercoinDelegation.ExistDelegationContract())
        ...
        bool verifiedDelegation = sbercoinDelegation.VerifyDelegation(address, delegation);
...
bool GetDelegationFeeFromContract(const uint160& address, uint8_t& fee)
{
    Delegation delegation;
    SbercoinDelegation& sbercoinDelegation = GetSbercoinDelegation();
    bool ret = sbercoinDelegation.GetDelegation(address, delegation);
    if(ret) ret &= sbercoinDelegation.VerifyDelegation(address, delegation);
    if(ret)
    {
        fee = delegation.fee;
    }
