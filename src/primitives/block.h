...
public:
    ...
    uint32_t nBits;
    uint32_t nNonce;
    uint256 hashStateRoot; // sbercoin
    uint256 hashUTXORoot; // sbercoin
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        READWRITE(nBits);
        READWRITE(nNonce);
        READWRITE(hashStateRoot); // sbercoin
        READWRITE(hashUTXORoot); // sbercoin
    ...
    void SetNull()
    {
        ...
        nBits = 0;
        nNonce = 0;
        hashStateRoot.SetNull(); // sbercoin
        hashUTXORoot.SetNull(); // sbercoin
    ...
    virtual bool IsProofOfStake() const //sbercoin
    {
    ...
    CBlockHeader& operator=(const CBlockHeader& other) //sbercoin
    {
        if (this != &other)
        {
...
public:
    ...
    std::pair<COutPoint, unsigned int> GetProofOfStake() const //sbercoin
    {
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits          = nBits;
        block.nNonce         = nNonce;
        block.hashStateRoot  = hashStateRoot; // sbercoin
        block.hashUTXORoot   = hashUTXORoot; // sbercoin
