...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setSbercoinSchedule(sbercoinDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = sbercoinDGP.getBlockSize(nHeight);
    minGasPrice = sbercoinDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = sbercoinDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    SbercoinTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractSbercoinTX resultConverter;
    if(!convert.extractionSbercoinTransactions(resultConverter)){
    ...
    for(SbercoinTransaction sbercoinTransaction : sbercoinTransactions){
        txGas += sbercoinTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + sbercoinTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(sbercoinTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, sbercoinTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= nCheckpointSpan)
        {
            ...
            sbercoinDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = sbercoinDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                sbercoinDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                sbercoinDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            sbercoinDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            sbercoinDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    SbercoinDelegation sbercoinDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= nCheckpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                sbercoinDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = sbercoinDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    sbercoinDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    sbercoinDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                sbercoinDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(sbercoinDelegations.GetDelegation(address, delegation) && SbercoinDelegation::VerifyDelegation(address, delegation))
                    {
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "sbercoinstake";
...
void StakeSbercoins(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
