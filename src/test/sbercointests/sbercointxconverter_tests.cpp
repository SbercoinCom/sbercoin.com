...
void checkResult(bool isCreation, std::vector<SbercoinTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    SbercoinTxConverter converter(transaction, NULL);
    ExtractSbercoinTX sbercoinTx;
    BOOST_CHECK(converter.extractionSbercoinTransactions(sbercoinTx));
    std::vector<SbercoinTransaction> result = sbercoinTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(sbercointxconverter_tests, TestingSetup)
