...
extern std::unique_ptr<SbercoinState> globalState;
...
inline void initState(){
    ...
    const std::string dirSbercoin = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<SbercoinState>(new SbercoinState(dev::u256(0), SbercoinState::openDB(dirSbercoin, hashDB, dev::WithExisting::Trust), dirSbercoin + "/sbercoinDB", dev::eth::BaseState::Empty));
...
inline dev::Address createSbercoinAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline SbercoinTransaction createSbercoinTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    SbercoinTransaction txEth;
    if(recipient == dev::Address()){
        txEth = SbercoinTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = SbercoinTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<SbercoinTransaction> txs){
